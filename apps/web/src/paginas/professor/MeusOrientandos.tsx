...
export function MeusOrientandos() {
  ...
  const [recusa, setRecusa] = useState<{ tccId: string; tipo: 'monografia' | 'continuidade' } | null>(null);
  ...
  return (
    <>
      ...
      {recusa && (
        <Modal
          titulo={recusa.tipo === 'monografia' ? 'Pedir ajustes na monografia' : 'Descontinuar o TCC'}
          subtitulo={recusa.tipo === 'monografia' ? 'O aluno poderá reenviar uma nova versão.' : 'Atenção: isso encerra o TCC como descontinuado.'}
          aoFechar={() => !enviando && setRecusa(null)}
        >
          {erro && <div className="erro-geral">{erro}</div>}
          <label className="campo">
            <span>{recusa.tipo === 'monografia' ? 'O que precisa ser ajustado' : 'Motivo da descontinuação'}</span>
