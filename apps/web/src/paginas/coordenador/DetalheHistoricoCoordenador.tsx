// Detalhe do Histórico do coordenador — SOMENTE LEITURA. Reaproveita os blocos de exibição
// (notas finais, timeline, banca e notas, documentos), mas NÃO mostra nenhuma ação de fluxo
// (validar, formar banca, solicitar ajuste, aprovar avaliação, liberar fase, editar avaliação,
// enviar documento, etc.) nem o botão "Excluir TCC". Dados de /tccs/historico-coordenador
// (escopado pelo JWT); o coordenador vê tudo (notas não sanitizadas).
...
export function DetalheHistoricoCoordenador() {
  ...
  if (!tcc) {
    return (
      <div className="det-cabecalho">
        <button className="det-voltar" onClick={() => navigate('/coordenador/historico')}>{icoVoltar} Voltar para o histórico</button>
        <section className="cartao-secao bloco"><p className="nota-vazio">TCC não encontrado no histórico.</p></section>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      {/* Notas Finais — coordenador vê tudo, com os pesos reais do semestre. */}
      <CardNotasFinais tcc={tcc} coordenador pesoF1={tcc.pesoFase1} pesoF2={tcc.pesoFase2} />
