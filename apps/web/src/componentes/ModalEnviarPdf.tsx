...
// Modal genérico de envio de PDF (monografia, versão final, etc.).
export function ModalEnviarPdf({ endpoint, titulo, subtitulo, rotulo, aoFechar, aoEnviado }: Props) {
  ...
  async function enviar() {
    setErro('');
    if (!arquivo) return setErro('Selecione o PDF.');
  ...
  return (
    <Modal titulo={titulo} subtitulo={subtitulo} aoFechar={() => !enviando && aoFechar()}>
      ...
      <CampoArquivo rotulo={rotulo} arquivo={arquivo} aoMudar={setArquivo} dica="PDF, até 10MB" />
      <div className="acoes">
